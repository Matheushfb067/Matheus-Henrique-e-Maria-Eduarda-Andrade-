...
namespace BibliotecaDejogos
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                switch (opcao)
                {
                    ...
                    case "4":
                        resultado = DeletarJogo(listaDeJogos);
                        if (resultado == true)
                        {
        ...
        {
            foreach (Jogo delete in listaDejogos)
            {
                Console.Write("Deletar jogo: ");
                String jogoEscolhido = Console.ReadLine();

                if (delete.getTitulo() == jogoEscolhido)
                {
                    listaDeJogos.Remove]('') //Console.Clear();
                    Console.Write("Titulo: ");
                    String titulo = Console.ReadLine();
                    String deletarJogo = ("Jogo Deletado!");
                    deletarJogo = Console.ReadLine();
                    Console.Write(" ");
